...
for path in `find ${LORE_PROC_CLANG_PATH} -iname '*.c'`; do
    ...
    if [ -e ${file_prefix}_params.txt ]; then
        while read -r params; do
            ...
            if ! popsicle-compile-unroll.sh ${file_prefix} "${params}" "nounroll"; then break; fi

            echo "[$file_i/$file_count] Running $name $params ..."

            for trial in `seq ${trials}`; do
                if res=$(timeout 10 ${PAPI_UTILS_PATH}/exec_loop_u ${papi_events_list}); then
            ...
            for trial in `seq ${trials}`; do
                if res=$(timeout 10 ${PAPI_UTILS_PATH}/exec_loop_n ${papi_events_list}); then
